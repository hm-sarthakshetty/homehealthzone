...
pros:
  ...
  - "Home Medix Claro memory-card reporting adds therapy summaries, trends, six aligned waveform panels, and event-window analysis"
cons:
  ...
  - "No Wi-Fi, cloud, or Bluetooth — Claro requires manual memory-card import and cannot feed an AirView-style live clinic dashboard"
...
**Claro clinical reporting — yes, via memory card.** Home Medix Claro imports the HM-BV-30 data on a Windows or macOS computer and presents therapy summaries, multi-night trends, six aligned nightly panels (events, pressure, airflow, leak, snoring, rolling AHI), and event-window analysis. This closes the earlier offline-reporting gap, but it is not automatic cloud telemetry or remote titration.

**Regulatory — ISO 9001 + ISO 13485 + CDSCO approved; no CE, no FDA listed.** CDSCO is sufficient for Indian retail and home channels. The CE and FDA gaps matter for hospital-channel procurement and international travel clearance ([CDSCO Medical Device Registry](https://cdscomdonline.gov.in/NewMedDev/Homepage)).
...
The prescriptions that need TVAPS are narrow but clinically important. Obesity-hypoventilation syndrome (OHS) at BMI 40-plus with daytime hypercapnia is the biggest indication; volume-assured home NIV is standard of care for severe OHS where fixed-pressure BiPAP leaves residual hypercapnia ([British Thoracic Society](https://www.brit-thoracic.org.uk/quality-improvement/guidelines/)). Progressive neuromuscular disease — Duchenne, ALS, post-polio — is the second; as muscle strength declines the pressure needed to deliver stable tidal volume rises over months and volume-assured modes adjust automatically. Severe COPD with chronic CO₂ retention after hospitalisation is the third ([Murphy PB et al, JAMA 2017](https://pubmed.ncbi.nlm.nih.gov/?term=Murphy+PB+JAMA+2017+home+NIV+COPD)). For these prescriptions the HM-BV-30 is not a nice-to-have — it is the mode selection.
...
**Patients whose clinic requires automatic cloud-reported compliance.** Claro provides detailed memory-card reports, but there is no published Wi-Fi, Bluetooth, automatic cloud upload, or AirView/Care Orchestrator integration. The card must reach a Claro workstation for review.
...
Score it 8.0 out of 10. Points for TVAPS, the full mode set, the 1.45 kg chassis with integrated heated humidifier, Claro memory-card reporting, and the CDSCO plus ISO 9001 plus ISO 13485 paperwork. Points off for the missing CE and FDA listings, absent automatic cloud/Bluetooth workflow, missing heated-tube and climate-control comfort stack, and absence of ASV-class central-apnea treatment. For simple fixed-CPAP OSA, buy the HM-CV-20. For ASV-needing patients, escalate to ResMed or Philips. For TVAPS-needing home-NIV patients at a sub-premium budget, this is the right Indian-market BiPAP.
