...
verdict: "Philips has discontinued the Everflo worldwide, so it is no longer a current buy despite leading this matchup on the specs below — for an in-production 5 LPM unit, the Home Medix HM-KV is the available alternative. The Philips Everflo wins for clinical home use — it carries the OPI, full alarm suite, US FDA and CE approvals, and the widest Indian service network. The Veayva is only worth considering if the Everflo is unavailable and the user cannot wait."
winner: "tie"
