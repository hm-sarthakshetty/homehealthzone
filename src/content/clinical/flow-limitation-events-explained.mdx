...
answer: "Flow limitation is partial upper-airway narrowing that flattens the inspiratory waveform without necessarily meeting apnea or hypopnea criteria. Repeated limitation can increase breathing effort, trigger RERAs, and fragment sleep, so a low AHI does not always prove that therapy is fully optimized. Device flags are estimates and must be read with symptoms, leak, and the full waveform or sleep study."
faqs:
  - q: "What is the difference between flow limitation and hypopnea?"
    a: "Flow limitation describes the shape and restriction of inspiration. A hypopnea must cross a defined airflow-reduction threshold and meet an oxygen-desaturation or arousal rule in a sleep study."
  - q: "Can a CPAP machine detect flow limitation?"
    a: "Many modern APAP devices estimate it from the flow waveform and may raise pressure in response, but brands report it differently and the result is not equivalent to a scored sleep study."
  - q: "Should I increase CPAP pressure for flow limitation?"
    a: "Not solely from one graph. Leak, nasal obstruction, sleep position, event type, comfort, and central apnea risk all matter; review persistent symptoms or limitation with the treating clinician."
---
