...
**1–5 LPM continuous flow at 90–96% purity.** Standard 5 LPM clinical category. The 90% floor is ISO-window adequate ([ISO 80601-2-69](https://www.iso.org/standard/73645.html)), the 96% ceiling is on par with better-specced peers, and the OPI on board means the household has a visual indicator if purity drifts. BPL does not publish per-LPM purity curves, which is the norm at this price — but the brand's pedigree in electrocardiography, ventilators and patient monitors means the QC discipline on this unit is plausibly more robust than unbranded budget competitors.
