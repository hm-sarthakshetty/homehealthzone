...
CDSCO import registration for Class-B medical devices like oxygen concentrators is now required under the 2022 medical-devices framework. Yuwell's India importer status should be confirmed on a per-batch basis — ask the seller for the CDSCO import licence number and the importer's address before you buy. ([CDSCO](https://cdsco.gov.in/opencms/opencms/en/Medical-Device-Diagnostics/Medical-Device-Diagnostics/))
